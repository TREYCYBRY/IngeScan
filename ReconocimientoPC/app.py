...
def register():
    if request.method == 'POST':
        ...
        if cursor.fetchone():
            flash('⚠ El código de estudiante ya está registrado.')
            conn.close()
...
def index():
    ...
    if request.method == 'POST':
        ...
        if not detecciones_validas:
            flash('⚠ La imagen no contiene un componente electrónico con la confianza mínima requerida.')
            return redirect(url_for('index'))

        # Tomamos la detección con mayor confianza
        box_confiable = max(detecciones_validas, key=lambda b: float(b.conf[0]))

        clase_id = int(box_confiable.cls[0])
